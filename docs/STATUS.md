...
| Text wrapping (`--wrap`, `--columns`) | ✅ | `--wrap=auto\|none\|preserve` and `--columns` (the fill width, default 72) are honored by every writer that lays out lines. A few constructs still mis-account width when reflowed at narrow columns — line blocks, footnote bodies, the AsciiDoc list-marker indent, and roff line-continuation — and grid-table column widths split the budget evenly rather than proportionally |
