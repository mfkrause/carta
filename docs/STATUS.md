...
| Syntax highlighting | ✅ | Code blocks and inline code spans are tokenized against embedded KDE syntax definitions and colored with one of the embedded styles (`--highlight-style NAME` or a theme file; default `pygments`). Renderers: HTML family (token `<span>`s plus an embedded stylesheet; inline code becomes a `code.sourceCode` span), LaTeX/Beamer (per-token macros in a `Highlighting` environment for blocks and a `\VERB` group for inline code), and DOCX (colored runs). `--syntax-highlighting` chooses the presentation: `default` colorizes, `none` (or `--no-highlight`) leaves code plain, `idiomatic` defers to the target's own listing construct (LaTeX/Beamer `lstlisting`/`\lstinline` via the `listings` package; a plain classed block elsewhere). `--number-lines`/`.numberLines` with `startFrom` are honored. `--syntax-definition FILE` adds a language; introspection via `--list-highlight-languages`, `--list-highlight-styles`, `--print-highlight-style NAME`. Class-name resolution uses the class as written (no alias canonicalization), so an exotic mixed-case spelling of a language alias may color differently from the lowercase form. |
