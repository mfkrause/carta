...
| 008 | Finish the HTML reader (extension toggles, footnote reconstruction, shared inline-scanner module) | P1 | L | 006 (soft) | DONE (on `worktree-finish-html-reader`, 9 commits `da065af`..`4d5e238`. Extension toggles, smart/`tex_math_*` via a shared inline-scanner, always-on footnote `Note` reconstruction, and the `gfm_auto_identifiers` stretch (implemented, not deferred — pinned to the oracle). The one residual was then closed: a body-level `<style>` is kept verbatim as a `RawInline` paragraph (the 2026-06-24 gap note was inaccurate — not blank-line-gated, and unrecognized elements were already at parity). Full conformance suite green across all surfaces; reader flipped to ✅ — full parity. Awaiting operator merge) |
