//! Writer-parity tests exercising the HTML writer across the full document model — every block and
...
//! the oracle to mint both the AST (as JSON our writer consumes) and the expected HTML, then diffs
//! our writer's output byte-for-byte. Expected values are minted at run time, never committed; the
//! oracle is hard-required (its absence fails with provisioning instructions rather than skipping).
...
#[test]
fn writer_matches_oracle_html_across_the_model() {
    assert!(
        pandoc_bin().is_file(),
    ...
    for case in cases() {
        match differential::writer("html", case.from, case.input).expect("run writer surface") {
            ...
            Diff::OracleRejected { stderr } => {
                failures.push(format!("{}: oracle rejected input: {stderr}", case.label));
            }
        }
    }
