...
use carta_ast::{
    Alignment, Attr, Block, Cell, ColSpec, ColWidth, Document, Format, Inline, ListAttributes, Row,
    Table,
};
...
impl State {
    ...
    fn grid_table(&mut self, table: &Table) -> String {
        ...
        let head_layout = place_columns(&head, columns);
        let body_layout = place_columns(&body, columns);
        let foot_layout = place_columns(&foot, columns);
        ...
        let colspans: Vec<(usize, usize)> = [&head_layout, &body_layout, &foot_layout]
            ...
            .collect();
        let content = grid_content_widths(&table.col_specs, &natural, &minword, &colspans, columns);
    ...
    fn measure_grid(
        ...
        for (row_index, row) in rows.iter().enumerate() {
            for (cell_index, cell) in row.cells.iter().enumerate() {
                ...
                let (width, word) = measure_lines(&lines);
    ...
    fn grid_rows(
        ...
        for (row_index, row) in rows.iter().enumerate() {
            ...
            for (cell_index, cell) in row.cells.iter().enumerate() {
                ...
                let width = merged_width(content, start, span);
...
fn place_columns(rows: &[&Row], columns: usize) -> Vec<Vec<(usize, usize)>> {
...
fn merged_width(content: &[usize], start: usize, span: usize) -> usize {
