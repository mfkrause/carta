...
impl Writer for BeamerWriter {
    fn write(&self, document: &Document, options: &WriterOptions) -> Result<String> {
        ...
        let units: Vec<String> = segment(&document.blocks, level)
            .iter()
            .map(|slide| render_slide(slide, level, options.wrap))
            .collect();
        Ok(units.join("\n\n"))
    }
...
fn render_slide(slide: &Slide, level: i32, wrap: WrapMode) -> String {
    match slide {
        Slide::Section { level, attr, title } => render_heading(*level, attr, title, wrap),
        Slide::Frame { title, body } => render_frame(title.as_ref(), body, level, wrap),
    }
}
...
fn render_frame(title: Option<&FrameTitle>, body: &[Block], level: i32, wrap: WrapMode) -> String {
    ...
    let rendered = render_body(body, level + 1, wrap);
...
fn render_body(blocks: &[Block], block_level: i32, wrap: WrapMode) -> String {
    let Some(group_level) = shallowest_header(blocks) else {
        return render_fragment(blocks, Dialect::SLIDE, wrap);
    };
    ...
    let units: Vec<String> = group_headings(blocks, group_level)
        ...
        .map(|heading| match heading {
            Heading::Loose(run) => render_fragment(run, Dialect::SLIDE, wrap),
            Heading::Section { attr, title, body } => {
                render_block_env(attr, title, body, group_level, wrap)
            }
        })
...
fn render_block_env(
    ...
    let rendered = render_body(body, group_level + 1, wrap);
    if !rendered.is_empty() {
        lines.push(rendered);
    }
