...
/// Whether `url` is a bare URI eligible to stand as an autolink: a recognized scheme followed by a
/// run made up solely of URI-permitted characters with well-formed percent-escapes. Non-ASCII
/// characters are admitted, since a bare address may carry them literally. The autolink-capable
/// writers share this one test so their recognition cannot drift.
pub(crate) fn is_uri(url: &str) -> bool {
    let Some(colon) = url.find(':') else {
        return false;
    };
    url.get(..colon).is_some_and(is_known_scheme) && is_percent_escaped_uri(url, true)
}
