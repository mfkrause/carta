...
Markdown tables to stdout; raw hyperfine JSON per case under `$BENCH_OUT` (gitignored). To refresh the committed headline document, regenerate `docs/BENCHMARKS.md` by hand and stamp it with this machine's specs, the carta commit, and the pandoc version.
